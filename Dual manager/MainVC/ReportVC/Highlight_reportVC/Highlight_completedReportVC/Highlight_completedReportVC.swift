//
//  Highlight_completedReportVC.swift
//  Dual manager
//
...
class Highlight_completedReportVC: UIViewController {
...
extension Highlight_completedReportVC: ASTableDataSource {
    ...
    func tableNode(_ tableNode: ASTableNode, didSelectRowAt indexPath: IndexPath) {
        ...
        self.performSegue(withIdentifier: "moveToHighlightReportProcessingVC1", sender: nil)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "moveToHighlightReportProcessingVC1"{
...
extension Highlight_completedReportVC: ASTableDelegate {
...
extension Highlight_completedReportVC {
    ...
    func retrieveNextPageWithCompletion( block: @escaping ([DocumentSnapshot]) -> Void) {
        ...
        let db = DataService.instance.mainFireStoreRef
            ...
            if lastDocumentSnapshot == nil {
                ...
                query = db.collection("video_report").whereField("status", isEqualTo: "Completed").order(by: "timeStamp", descending: true).limit(to: 10)
                
                
            } else {
                
                query = db.collection("video_report").whereField("status", isEqualTo: "Completed").order(by: "timeStamp", descending: true).limit(to: 10).start(afterDocument: lastDocumentSnapshot)
            }
